# Terms of Service

**EazyDone Handyman**

**Last Updated: February 2, 2026**

---

## 1. Introduction

Welcome to EazyDone Handyman ("Platform", "we", "us", or "our"). These Terms of Service ("Terms") govern your access to and use of our website, mobile application, and services that connect customers with independent handyman service providers.

By accessing or using our Platform, you agree to be bound by these Terms. If you do not agree to these Terms, please do not use our Platform.

---

## 2. Definitions

- **"Customer"** refers to any user who requests handyman services through the Platform.
- **"Handyman"** or **"Service Provider"** refers to independent contractors who offer handyman services through the Platform.
- **"Job"** refers to a service request created by a Customer and fulfilled by a Handyman.
- **"Service Fee"** refers to the amount payable to the Handyman for completing a Job.
- **"Platform Fee"** refers to the fee charged by EazyDone Handyman for facilitating the transaction.
- **"Total Amount"** refers to the combined Service Fee and Platform Fee paid by the Customer.

---

## 3. Platform Description

EazyDone Handyman is an online marketplace that connects Customers with independent Handymen for home repair and maintenance services. We do not directly provide handyman services. Handymen are independent contractors and not employees of EazyDone Handyman.

### 3.1 Our Role

We provide:
- A platform for Customers to request services
- A marketplace for Handymen to find work opportunities
- Secure payment processing
- Communication tools between parties
- Escrow services to protect both parties

### 3.2 What We Are Not

- We are not a handyman service provider
- We do not employ Handymen
- We do not guarantee the quality of work performed
- We do not supervise or control how Handymen perform their services

---

## 4. User Accounts

### 4.1 Account Registration

To use our Platform, you must:
- Be at least 18 years of age
- Provide accurate and complete registration information
- Maintain the security of your account credentials
- Notify us immediately of any unauthorized use

### 4.2 Customer Accounts

Customers can create accounts to:
- Request handyman services
- Track job progress
- Make secure payments
- Rate and review Handymen

### 4.3 Handyman Accounts

Handymen must complete our verification process, which includes:
- Identity verification
- Skills assessment
- Background screening (where applicable)
- Stripe Connect onboarding for payments

---

## 5. Payment Terms

### 5.1 Pricing

Service prices are displayed on our Platform and include:
- **Service Fee**: The base fee for the handyman service
- **Platform Fee**: A 10% fee charged on top of the Service Fee

**Example Pricing:**
| Component | Amount |
|-----------|--------|
| Service Fee | $100.00 |
| Platform Fee (10%) | $10.00 |
| **Total Amount** | **$110.00** |

### 5.2 Payment Processing

- All payments are processed securely through Stripe
- Payments are held in escrow until job completion is confirmed
- We accept major credit cards and debit cards

### 5.3 Payment Breakdown and Fees

When a Customer pays for a service, the Total Amount is distributed as follows:

| Recipient | Percentage | Description |
|-----------|------------|-------------|
| Payment Processor (Stripe) | ~3.4% + $0.50 | Third-party processing fee |
| EazyDone Handyman | ~9% of net | Platform fee for services |
| Handyman | ~88% of net | Service fee for work performed |

**Detailed Example ($110 Total):**
| Item | Amount |
|------|--------|
| Customer Pays | $110.00 |
| Stripe Processing Fee | -$4.24 |
| Net Amount | $105.76 |
| Platform Fee (from net) | $9.61 |
| **Handyman Receives** | **$96.15** |

*Note: Stripe processing fees may vary based on card type and region.*

### 5.4 Escrow and Fund Release

1. **Payment Authorization**: When a Customer confirms a job, payment is authorized and held securely.
2. **Job Completion**: The Handyman marks the job as complete upon finishing the work.
3. **Customer Confirmation**: The Customer confirms job completion via WhatsApp notification.
4. **Fund Release**: Upon confirmation, funds are released to the Handyman's account.

### 5.5 Payout Schedule

- Handymen receive payouts to their linked bank account
- Payouts are processed within 2-3 business days after fund release
- Minimum payout thresholds may apply

---

## 6. Job Process

### 6.1 Job Creation

1. Customer selects a service category
2. Customer provides job details and location
3. Customer reviews pricing and confirms booking
4. Payment is authorized and held in escrow

### 6.2 Job Assignment

1. Available Handymen can view and accept jobs
2. Customer may be notified of interested Handymen
3. Job is assigned to a qualified Handyman

### 6.3 Job Completion

1. Handyman performs the requested service
2. Handyman marks the job as complete
3. Customer receives confirmation request
4. Customer confirms completion or reports issues
5. Upon confirmation, payment is released

### 6.4 Job Statuses

| Status | Description |
|--------|-------------|
| Pending | Job created, awaiting Handyman |
| Accepted | Handyman assigned |
| In Progress | Work has begun |
| Pending Confirmation | Awaiting Customer confirmation |
| Completed | Job finished, payment released |
| Disputed | Issue reported, under review |
| Cancelled | Job cancelled |

---

## 7. Cancellation and Refunds

### 7.1 Customer Cancellations

- **Before Handyman Assignment**: Full refund
- **After Assignment, Before Work Begins**: Refund minus administrative fee
- **After Work Begins**: Partial refund based on work completed (at our discretion)

### 7.2 Handyman Cancellations

- Handymen may cancel before arriving at the job site
- Repeated cancellations may result in account suspension
- Customer will receive a full refund for Handyman cancellations

### 7.3 Refund Processing

- Refunds are processed within 5-10 business days
- Refunds are credited to the original payment method
- Stripe processing fees may not be refundable

---

## 8. Handyman Obligations

As a Handyman on our Platform, you agree to:

### 8.1 Service Quality
- Perform services professionally and competently
- Use appropriate tools and materials
- Complete jobs within agreed timeframes
- Communicate promptly with Customers

### 8.2 Legal Compliance
- Maintain all required licenses and permits
- Carry appropriate insurance coverage
- Comply with local laws and regulations
- Pay all applicable taxes on your earnings

### 8.3 Platform Rules
- Maintain accurate profile information
- Respond to job requests promptly
- Not solicit Customers outside the Platform
- Not accept payments outside the Platform

---

## 9. Customer Obligations

As a Customer on our Platform, you agree to:

### 9.1 Accurate Information
- Provide accurate job descriptions
- Specify correct service locations
- Disclose any relevant property conditions

### 9.2 Access and Safety
- Provide safe access to the work area
- Secure pets during service
- Be present or available during the job (if required)

### 9.3 Payment
- Pay the full amount for completed services
- Confirm job completion promptly
- Report issues through proper channels

---

## 10. Disputes and Resolution

### 10.1 Reporting Issues

If you are dissatisfied with a service:
1. Report the issue within 24 hours of job completion
2. Provide detailed description and evidence (photos if applicable)
3. Our team will review and respond within 48 hours

### 10.2 Resolution Process

- We will mediate disputes between Customers and Handymen
- Partial or full refunds may be issued at our discretion
- Repeated disputes may result in account review

### 10.3 Arbitration

For disputes that cannot be resolved through mediation, parties agree to binding arbitration in accordance with Singapore law.

---

## 11. Limitation of Liability

### 11.1 Platform Limitations

EazyDone Handyman is not liable for:
- Quality or outcome of services provided by Handymen
- Property damage caused by Handymen
- Personal injury during service provision
- Delays or cancellations by Handymen
- Technical issues or Platform downtime

### 11.2 Maximum Liability

Our maximum liability for any claim shall not exceed the Platform Fee paid for the relevant Job.

### 11.3 Indemnification

Users agree to indemnify and hold harmless EazyDone Handyman from any claims, damages, or expenses arising from:
- Violation of these Terms
- Violation of any law or third-party rights
- Content posted on the Platform

---

## 12. Intellectual Property

### 12.1 Platform Ownership

All content, features, and functionality of the Platform are owned by EazyDone Handyman and protected by intellectual property laws.

### 12.2 User Content

By posting content on our Platform, you grant us a non-exclusive, worldwide license to use, display, and distribute such content for Platform operations.

---

## 13. Privacy

Your privacy is important to us. Please review our Privacy Policy, which explains how we collect, use, and protect your personal information.

---

## 14. Modifications to Terms

We reserve the right to modify these Terms at any time. Changes will be effective upon posting to the Platform. Continued use of the Platform after changes constitutes acceptance of the modified Terms.

---

## 15. Termination

### 15.1 By Users

You may terminate your account at any time by contacting us. Outstanding payments must be settled before account closure.

### 15.2 By EazyDone Handyman

We may suspend or terminate accounts for:
- Violation of these Terms
- Fraudulent activity
- Repeated disputes or complaints
- Inactivity for extended periods

---

## 16. Governing Law

These Terms shall be governed by and construed in accordance with the laws of Singapore. Any disputes shall be subject to the exclusive jurisdiction of the courts of Singapore.

---

## 17. Contact Information

For questions about these Terms, please contact us:

**EazyDone Handyman**
- Email: easydonehandyman@gmail.com
- Website: [Your Website URL]

---

## 18. Severability

If any provision of these Terms is found to be unenforceable, the remaining provisions shall continue in full force and effect.

---

## 19. Entire Agreement

These Terms, together with our Privacy Policy, constitute the entire agreement between you and EazyDone Handyman regarding the use of our Platform.

---

*By using EazyDone Handyman, you acknowledge that you have read, understood, and agree to be bound by these Terms of Service.*
